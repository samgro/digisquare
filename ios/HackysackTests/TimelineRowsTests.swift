...
struct TimelineRowsTests {
    @Test func suggestionsFallIntoTheDayTheUserWasThere() throws {
        ...
        let rows = timelineRows(for: [yesterdaysSuggestion, saved])

        #expect(rows.count == 4)
        guard case .dayHeader = rows[0], case .entry(let first) = rows[1],
              case .dayHeader = rows[2], case .entry(let second) = rows[3] else {
