...
final class CheckinStore: ObservableObject {
    ...
    func loadTimeline() async {
        do {
            let serverCheckins = try await checkinsAPI.listCheckins(userId: CurrentUser.userId)
    ...
    func submit(place: Place, message: String?) {
        ...
        let entry = TimelineEntry(
            id: UUID(),
            draft: draft,
            checkin: Checkin(placeholderFor: draft),
            syncStatus: .saving
        )
...
private extension Checkin {
    ...
    init(placeholderFor draft: CheckinDraft) {
        let now = Date()
