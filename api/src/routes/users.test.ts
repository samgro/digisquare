...
describe("GET /:id/stats", () => {
  it("returns the counts and top places", async () => {
    controls.queue(
      [{ checkinCount: 7, placeCount: 3 }],
      [
        {
    ...
    const response = await get(`/${OTHER_USER_ID}/stats`);
  ...
  it("answers zeros for someone with no checkins", async () => {
    controls.queue([{ checkinCount: 0, placeCount: 0 }], []);

    const response = await get(`/${CURRENT_USER_ID}/stats`);

    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ checkinCount: 0, placeCount: 0, topPlaces: [] });
  });

  it("rejects an id that isn't a uuid", async () => {
    const response = await get("/not-a-uuid/stats");
